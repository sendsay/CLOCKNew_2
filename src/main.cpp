...
// Ticker getWeatherNow(getWeather, 10000, 0, MILLIS);
...
void loop()
{
    ...
    { // счетчик секунд и флаг для процессов                                            // на початку нової секунди скидаємо secFr в "0"
        lastSecond = second;
    ...
    if (config.weatherOn == 1 && (((minute % 29 == 0) ) and (second == 0) and (not secFr)))
    {
        if (WIFI_connected)
        {
            printTime();
    ...
    */
    ...
    if (second == 50 && config.mqttOn && !alarm_stat && WIFI_connected && secFr == 0)
    {
        ...
        {
            if (bmp280)
                MQTTclient.publish(config.mqttpubtemp, (String(t1) + "." + String(t2)).c_str());
            if (bmp280)
                MQTTclient.publish(config.mqttpubpress, (String(pressBmp).c_str()));
            if (si7021)
                MQTTclient.publish(config.mqttpubtempUl, (String(t3) + "." + String(t4)).c_str());
            if (si7021)
                MQTTclient.publish(config.mqttpubhum, (String(humSi7021)).c_str());
        }
        ...
        if (not secFr)
        {
            ...
            if (si7021)
                Serial.print("Temperature: " + String(t1) + "." + String(t2) + "*C,   ");
            if (bmp280)
                Serial.print("Na ulice: " + String(t3) + "." + String(t4) + "*C,   ");
            if (humSi7021 != 0)
                Serial.print("Humidity: " + String(humSi7021) + " %,  ");
            if (pressBmp != 0)
                Serial.print("  Pressure: " + String(pressBmp) + " mmHg,  Altitude: " + String(altBmp) + " m.");
            Serial.println("");
        }
    }
    ...
    {
        if (config.mqttOn)
            MQTTclient.loop(); // перевіряємо чи намає вхідних повідомлень, як є, то кoлбек функція
    }
...
void loadConfig(const char *filename, Config &config)
{
    ...
    if (doc["langWeather"] == "ua")
    {
        ukrText();
        lang = 0;
    } // Выбор языка для сообщений
    else if (doc["langWeather"] == "ru")
    {
        rusText();
        lang = 1;
    }
    else if (doc["langWeather"] == "en")
    {
        enText();
        lang = 2;
    }
...
void printStringWithShift(const char *s, int shiftDelay)
{
    ...
    { // коли працює ця функція, основний цикл зупиняється
        ...
        if (config.mqttOn)
            MQTTclient.loop(); // зберігаемо можливість слухати MQTT топік під час бігучої стоки
    }
}
...
void getNTPtime()
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (!cb)
            Serial.println("          no packet yet..." + String(i + 1));
        if (!cb && i == 2)
        { // якщо час не отримано
            statusUpdateNtpTime = 0;
    ...
    { // якщо отримали пакет з серверу
        ...
        if (month < 3 || month > 10)
            summerTime = false; // не переходимо на літній час в січні, лютому, листопаді і грудню
        if (month > 3 && month < 10)
            summerTime = true; // Sommerzeit лічимо в квіні, травні, червені, липні, серпені, вересені
...
void showSimpleTempU()
{
    ...
    if (t3 <= -10.0 || t3 >= 10)
        showDigit((t3 < 0 ? (t3 * -1) / 10 : t3 / 10), 4, dig5x8rn);
...
void getWeather()
{
    ...
    if (windDeg >= 345 || windDeg <= 22)
        windDegString = "\211"; //"Північний";
    if (windDeg >= 23 && windDeg <= 68)
        windDegString = "\234"; //"Північно-східний";
    if (windDeg >= 69 && windDeg <= 114)
        windDegString = "\230"; //"Східний";
    if (windDeg >= 115 && windDeg <= 160)
        windDegString = "\235"; //"Південно-східний";
    if (windDeg >= 161 && windDeg <= 206)
        windDegString = "\210"; //"Південний";
    if (windDeg >= 207 && windDeg <= 252)
        windDegString = "\232"; //"Південно-західний";
    if (windDeg >= 253 && windDeg <= 298)
        windDegString = "\231"; //"Західний";
    if (windDeg >= 299 && windDeg <= 344)
        windDegString = "\233"; //"Північно-західний";
...
void convertWeatherDes()
{
    if (weatherDescription == "clear sky")
        weatherDescription = tClearSky;
    else if (weatherDescription == "sky is clear")
        weatherDescription = tSkyIsClear;
    else if (weatherDescription == "few clouds")
        weatherDescription = tFewClouds;
    else if (weatherDescription == "scattered clouds")
        weatherDescription = tScatteredClouds;
    else if (weatherDescription == "broken clouds")
        weatherDescription = tBrokenClouds;
    else if (weatherDescription == "overcast clouds")
        weatherDescription = tOvercastClouds;
    else if (weatherDescription == "light rain")
        weatherDescription = tLightRain;
    else if (weatherDescription == "moderate rain")
        weatherDescription = tModerateRain;
    else if (weatherDescription == "light intensity shower rain")
        weatherDescription = tLightIntensityShowerRain;
    else if (weatherDescription == "shower rain")
        weatherDescription = tShowerRain;
    else if (weatherDescription == "heavy intensity rain")
        weatherDescription = tHeavyIntensityRain;
    else if (weatherDescription == "very heavy rain")
        weatherDescription = tVeryHeavyRain;
    else if (weatherDescription == "thunderstorm")
        weatherDescription = tThunderstorm;
    else if (weatherDescription == "haze")
        weatherDescription = tHaze;
    else if (weatherDescription == "fog")
        weatherDescription = tFog;
    else if (weatherDescription == "mist")
        weatherDescription = tMist;
    else if (weatherDescription == "shower sleet")
        weatherDescription = tShowerSleet;
    else if (weatherDescription == "light snow")
        weatherDescription = tLightSnow;
    else if (weatherDescription == "light shower snow")
        weatherDescription = tLightShowerSnow;
    else if (weatherDescription == "snow")
        weatherDescription = tSnow;
}
...
void callback(char *topic, byte *payload, unsigned int length)
{
    ...
    if (String(topic) == config.mqttbutt)
    { //Кнопка
        bip();
    ...
    if (String(topic) == config.mqttsub)
    {
        ...
        { // в payload[0] - хранится первый полученный символ. 48, 58 и 45 - это коды знаков можете их посмотреть в fontUA_RU_PL_DE[]
            if (payload[0] == 45)
            { // если первый символ = "-" (равен минусу) то tMqtt5 = -1
                MQTTClientas.tMqtt5 = -1;
                if (payload[1] >= 48 && payload[1] < 58)
                {                                          //  здесь проверяем уже второй символ что он является числом...
                ...
                if (payload[3] == 46)
                { // если третий знак не число, то проверяем его на то что но является точкой...
                    if (payload[4] >= 48 && payload[4] < 58)
                        MQTTClientas.tMqtt4 = payload[4] - 48; // если третий знак точка и четвертый знак является числом, то это десятые значения
                }
                if (payload[2] == 46)
                { // тоже самое со втрорым знаком...
                    if (payload[3] >= 48 && payload[3] < 58)
                        MQTTClientas.tMqtt4 = payload[3] - 48;
                }
            }
            else
            { // здесь таже самая процедура но уже с положительными числами))))) tMqtt5 = 1 - это признак того что число положителньное или отрицательное....
                MQTTClientas.tMqtt5 = 1;
                ...
                {
                    if (payload[2] >= 48 && payload[2] < 58)
                        MQTTClientas.tMqtt4 = payload[2] - 48;
                }
            }
        }
    }
